...
class VerifierService:
    ...
    def verify(self, answer: str, rows: list[dict]) -> bool:
        ...
        if self.settings.gemini_api_key:
            result = self._gemini_verify(answer, rows)
            if result is not None:
                return result
        return self._local_verify(answer, rows)

    def _gemini_verify(self, answer: str, rows: list[dict]) -> bool | None:
        evidence = "\n\n".join(
        ...
        prompt = (
            "You are a strict verifier for an insurance policy AI answer. "
